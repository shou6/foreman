...
- **渡すもの**：エディタの選択範囲、問題パネルのエラーと警告、未コミットの `git diff`、ダイアログで選んだファイルを指示に添える。クリップボードのスクリーンショットは貼り付けると画像として添付できる。ファイルの右クリック「Foreman: タスクに添付」と、エディタの右クリック「Foreman: 選択範囲をタスクに添付」も使える。タスク画面へのドロップもできる（エディタ領域からは Shift を押しながらドラッグ）
- **通知とステータスバー**：入力待ちと完了が分かる。ステータスバーに実行中と入力待ちの件数が出る
- **指示のプリセットと Context パネル**：指示の先頭に `/fix`、`/test`、`/review` と書くとプリセットの本文に置き換わる（設定 `foreman.presets` で変えられる）。入力欄の上の「Claude に渡す内容」を開くと、添付を含めて実際に送る文と、ディレクトリ、モデル、承認方式、常に許可しているルールが見える
- **コンテキストのメーター**：タスク画面、サイドバー、ステータスバーに、コンテキストの窓をどれだけ使っているかを出す。ターンごとに入力と出力のトークン数が出る
...
- 承認の取り消しと、承認済みの差分カードの印
- シナリオテストを増やし、開発ホストを使う手作業の確認を減らす
