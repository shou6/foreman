...
- **Prompt presets, commands and Context panel**: type `/fix`, `/test` or `/review` at the start of a prompt to expand a preset (edit them in `foreman.presets`). The `/` list also offers your Claude Code commands and skills, read from your Claude Code once at startup, and Claude Code runs them as usual. The "What Claude will receive" line above the input shows the directory, the permission mode and the number of attachments. Open it to see the exact text Foreman will send and the rules you always allowed.
