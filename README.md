...
- **Prompt presets, commands and Context panel**: type `/fix`, `/test` or `/review` at the start of a prompt to expand a preset (edit them in `foreman.presets`). Keep typing after `/` to filter your Claude Code commands and skills (read from your Claude Code once at startup, matched by prefix on any `:`-separated part), pick one with the arrow keys and Enter, and Claude Code runs it as usual. The "What Claude will receive" line above the input shows the directory, the permission mode and the number of attachments. Open it to see the exact text Foreman will send, the rules you always allowed and, while Claude Code is running for the task, the state of your MCP servers.
