...
- **Notifications and status bar**: get a notification when a task needs you, has changes to review, or failed. The status bar counts the running tasks and shows the model and context usage of the task you are looking at. Hover over it to see how many tasks wait for you, and click it to open the task list.
...
- **Plan usage**: with a Pro or Max plan, the sidebar and the status bar show how much of your 5-hour, 7-day and per-model limits you have used, and the sidebar adds when they reset. In the status bar they have their own item, labeled `Claude:`. Choose what each place shows with `foreman.planUsage.sidebar` and `foreman.planUsage.statusBar`. The status bar shows them as soon as Visual Studio Code starts, before you open Foreman. To keep them out of the status bar, turn off `foreman.planUsage.showInStatusBar`; Foreman then reads them only after you open a Foreman view. Foreman reads the figures from your Claude Code at startup, after a turn ends, every 10 minutes, and when you click the refresh icon in the sidebar. Reading them uses none of your limits.
...
- **Git worktrees**: run a task in its own worktree and branch. For a worktree task, the "Finish" section in the secondary side bar walks you through approving the changes, reviewing the whole diff and merging into the branch you started from. You can also discard the worktree there. Worktrees live in `.foreman/worktrees` inside the repository. A new worktree has only the files that Git tracks, so Foreman can copy files such as `.env` into it and run a setup command such as `npm install` before the task starts. If the setup fails, Foreman shows a warning and starts the task anyway.
...
| `foreman.worktreeCopyFiles` | Glob patterns, relative to the repository root, of files to copy into a new task worktree, such as `.env`. Use it for files that Git does not track. Default: none. |
| `foreman.worktreeSetupCommand` | Shell command to run in a new task worktree before the task starts, such as `npm install`. Default: none. |
...
| `foreman.planUsage.showInStatusBar` | Show the plan usage in the status bar. When off, the status bar ignores `foreman.planUsage.statusBar`, and Foreman reads the plan usage only after you open a Foreman view. Default: on. |
